...
import {
  scanInboxWithPeriod, importFiles, getEmployeeFiles, previewFileUrl, previewEml,
      ...
      <div style={{
        ...
        <div className="dropdown" ref={writeDD.ref}>
          ...
          <div className={`dropdown-menu ${writeDD.open ? "open" : ""}`}>
            <div className="dropdown-label">{selEmp ? selEmp.cn || selEmp.en : "請先選員工"}</div>
            {selEmp ? <>
            ...
            </> : <div className="dropdown-item" style={{ color: "#888", cursor: "default", fontSize: 11 }}>點選員工後可使用</div>}
          </div>
        </div>
        ...
        <div className="dropdown" ref={verifyDD.ref}>
          ...
          <div className={`dropdown-menu ${verifyDD.open ? "open" : ""}`}>
            <div className="dropdown-label">{selEmp ? selEmp.cn || selEmp.en : "請先選員工"}</div>
            {selEmp ? <>
              <button className="dropdown-item" onClick={() => { verifyDD.setOpen(false); setModal({ type: "verify" }); }}>✈️ Travel</button>
              <button className="dropdown-item" onClick={() => { verifyDD.setOpen(false); setModal({ type: "verify" }); }}>⏰ OT</button>
              <button className="dropdown-item" onClick={() => { verifyDD.setOpen(false); setModal({ type: "verify" }); }}>🌙 NS</button>
              <button className="dropdown-item" onClick={() => { verifyDD.setOpen(false); setModal({ type: "verify" }); }}>📅 ESS</button>
              <div className="dropdown-sep" />
              <button className="dropdown-item" onClick={() => { verifyDD.setOpen(false); setModal({ type: "verify" }); }}>🔍 全部核對</button>
            </> : <div className="dropdown-item" style={{ color: "#888", cursor: "default", fontSize: 11 }}>點選員工後可使用</div>}
          </div>
        </div>
      </div>
